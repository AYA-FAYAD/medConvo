...
const seedDB = async () =>{
    ...
    for(let i=1; i <=50; i ++ ){
        ...
        const camp = await prisma.conferenceschema.create({data:{location:`${cities[random1000].city},
        ${cities[random1000].state}`, 
        title:`${sample(descriptors)} ${sample(places)}`,
        authors: { connect: { id: 7 } } ,
        image:'https://source.unsplash.com/collection/483251',
        description:'Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis, suscipit? Cumque enim rerum aperiam, est quis tempore perferendis illo rem veniam? Quas id aperiam alias veritatis ut quis voluptatem rerum.',
        price: price
